...
Pac_data_pca <- prcomp(na.omit(vibrio[,2:14]), scale = TRUE)
print(Pac_data_pca)
...
png(filename = "Pac_PCA_Dim.png",width = 20, height = 20, units = "cm", res=300, pointsize = 0.1)
...
Pac_graf01<-factoextra::fviz_eig(Pac_data_pca,addlabels = TRUE,hjust = -0.3,linecolor ="red")+
  labs(title="Pacific PCA - Screeplot",x="Dimensions (PC)", y="% variable explained.")+
  ylim(c(0,65))
...
Pac_PCA_23<-factoextra::fviz_pca_var(Pac_data_pca, 
                                 ...
                                 gradient.cols = c("#2c7fb8", "#ffeda0", "#f03b20"),
                                 ggtheme = theme_minimal())+
  labs(x="PC2 (13.8%)", y="PC3 (9.2%)")




png(filename = "Pac_PCA_total.png",width = 20, height = 20, units = "cm", res=300, pointsize = 8)
