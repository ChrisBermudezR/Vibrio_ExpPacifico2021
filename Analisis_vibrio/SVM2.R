...
########################
vibrio=read.csv("DatosTotal.csv")
library(e1071)
clasificador(vibrio$Temperatura, vibrio$Salinidad, vibrio$MareaFactor, "Temperatura", "Salinidad")



clasificador<-function(variable_1, variable_2, factor_1, nombre_1, nombre_2){
...
fit = svm(factor(factor_1) ~ ., data = dat, scale = FALSE, kernel = "radial", cost = 5)
print(fit)
summary(fit)
